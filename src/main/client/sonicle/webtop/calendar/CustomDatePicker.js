...
Ext.define('Sonicle.webtop.calendar.CustomDatePicker', {
	extend: 'Ext.picker.Date',
	parent_panel:null,
	item_datepicker:null,
        first:true,
        startDay:1,
	renderTpl: [
				'<div id="{id}-innerEl" data-ref="innerEl">',
					'<div style="height: 27px;" class="{baseCls}-header">',
						'<div id="{id}-prevEl" data-ref="prevEl" class="{baseCls}-prev {baseCls}-arrow" role="button" title="{prevText}" style="display: none;"></div>',
						'<div id="{id}-middleBtnEl" data-ref="middleBtnEl" class="{baseCls}-month" role="heading">{%this.renderMonthBtn(values, out)%}</div>',
						'<div id="{id}-nextEl" data-ref="nextEl" class="{baseCls}-next {baseCls}-arrow" role="button" title="{nextText}" style="display: none;"></div>',
					'</div>',
					...
					'</table>',
					 '<tpl if="showToday">',
						'<div id="{id}-footerEl" data-ref="footerEl" role="presentation" class="{baseCls}-footer" style="display: none;>{%this.renderTodayBtn(values, out)%}</div>',
					'</tpl>',
				'</div>',
				{
	...
	initComponent: function() {
        ...
        me.dayNames = me.dayNames.slice(me.startDay-1).concat(me.dayNames.slice(0, me.startDay-1));
    ...
    onOkClick: function(picker, value) {
        ...
        me.setValue(date);
        this.parent_panel.datepicker1.setValue(Ext.Date.add(date, Ext.Date.MONTH, -1));
        me.hideMonthPicker();
    },
    ...
    selectedUpdate: function(date) {
        ...
        if (!me.first){    
            ...
            var cell_item_picker=this.parent_panel.datepicker1.activeCell;
            if (cell_item_picker){
            ...
            if (this.parent_panel.datepicker1.cells){
                for (c = 0; c < this.parent_panel.datepicker1.cells.getCount(); c++) {
                    cell = this.parent_panel.datepicker1.cells.item(c)
                    Ext.fly(cell).removeCls(cls);
                }
            }
	...
	this.parent_panel.datepicker1.setValue(Ext.Date.add(this.parent_panel.datepicker1.activeDate, Ext.Date.MONTH, -1));
    ...
    update: function(date, forceRefresh) {
        ...
        if (me.rendered) {
            ...
            if(!forceRefresh && active && me.el && active.getMonth() == date.getMonth() && active.getFullYear() == date.getFullYear()){
                ...
                if (date && active && date.getMonth() < active.getMonth() && me.parent_panel.datepicker1 &&  me.parent_panel.datepicker1.activeDate) me.parent_panel.datepicker1.fullUpdate((Ext.Date.add(date, Ext.Date.MONTH, -1)),me.parent_panel.datepicker1.activeDate);
                else if (active && date && date.getMonth() > active.getMonth() && me.parent_panel.datepicker1 &&  me.parent_panel.datepicker1.activeDate) me.parent_panel.datepicker1.fullUpdate((Ext.Date.add(date, Ext.Date.MONTH, -1)),me.parent_panel.datepicker1.activeDate);
            }
        }
        return me;
    }
