...
public class OEventRecurrence extends EventsRecurrences {
	...
	public void resize(LocalDate untilDate, LocalTime untilTime, DateTimeZone untilTimezone) {
		Recur recur = getRecurrenceObject();
		ICal4jUtils.setRecurUntilDate(recur, untilDate.toDateTime(untilTime, untilTimezone));
		setRule(recur.toString());
		setUntil(untilDate.toDateTimeAtStartOfDay(untilTimezone).plusDays(1));
	}
