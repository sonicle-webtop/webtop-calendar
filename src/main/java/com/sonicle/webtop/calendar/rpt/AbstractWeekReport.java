...
public abstract class AbstractWeekReport extends AbstractReport {
	...
	public void setDataSource(CalendarManager manager, DateTime fromDate, DateTime toDate, DateTimeZone utz, List<CalendarManager.CalendarEvents> calendarEvents) throws WTException {
		...
		for(int i=0; i<days; i++) {
			daysEvents.add(new ArrayList<RBAgendaEvent>()); // Inits internal array
			...
			for(int i=0; i<days; i++) {
				...
				if(isInDay(utz, dayDateFrom, se)) {
					...
					if(startsInDay(utz, dayDateFrom, se) && endsInDay(utz, dayDateFrom, se)) {
						...
						if(startsInDay(utz, dayDateFrom, se)) spanRight = DateTimeUtils.datesBetween(dayDateFrom, se.getEndDate().withZone(utz));
						if(endsInDay(utz, dayDateFrom, se)) spanLeft = DateTimeUtils.datesBetween(se.getStartDate().withZone(utz), dayDateFrom);
					}
		...
		for(int i=0; i<dayDates.size(); i++) {
			dayItems.add(new BeanDayAgenda(dayDates.get(i), daysEvents.get(i)));
		}
