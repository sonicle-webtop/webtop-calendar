 ...
 * @author malbinola
 */
public class EventUIConditionBuildingVisitor extends EventConditionBuildingVisitor {
	protected DateTime rangeStart = null;
	protected DateTime rangeEnd = null;
	
	public EventUIConditionBuildingVisitor() {
		super();
	...
	protected Condition buildCondition(String fieldName, Operator operator, Collection<?> values) {
		if (EventQueryUI.AFTER.equals(fieldName)) {
			rangeStart = singleValueAsDateTime(values);
			return trueCondition();
			
		} else if (EventQueryUI.BEFORE.equals(fieldName)) {
			rangeEnd = singleValueAsDateTime(values);
			if (rangeEnd != null) rangeEnd = rangeEnd.plusDays(1);
			return trueCondition();
			
		} else if (EventQueryUI.ANY.equals(fieldName)) {
			return defaultCondition(EVENTS.TITLE, operator, values)
				.or(defaultCondition(EVENTS.DESCRIPTION, operator, values));
