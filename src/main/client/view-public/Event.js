 ...
 */
Ext.define('Sonicle.webtop.calendar.view-public.Event', {
	...
	requires: [
		...
		'WTA.ux.panel.Panel',
		'Sonicle.webtop.calendar.model-public.Event'
	],
					...
					items: [
						{
							...
							growMax: 150,
							hideEmptyLabel: true,
							emptyText: me.res('pub.event.fld-description.et'),
							anchor: '100%',
							listeners: {
							...
							layout: 'hbox',
							fieldLabel: me.res('pub.event.fld-when.lbl'),
							items: [
								{
									xtype: 'displayfield',
									bind: '{record.when}'
							...
							xtype: 'fieldcontainer',
							layout: 'hbox',
							fieldLabel: me.res('pub.event.fld-where.lbl'),
							items: [
								{
									xtype: 'displayfield',
									bind: '{record.where}',
							...
							xtype: 'fieldcontainer',
							layout: 'fit',
							fieldLabel:  me.res('pub.event.fld-attendees.lbl'),
							items: [
								{
									xtype: 'gridpanel',
									bind: {
	...
	privates: {
		linkModel: function() {
			...
			vm.linkTo(linkName, {
				type: 'Sonicle.webtop.calendar.model-public.Event',
				create: true
			});
			if(data) {
				mo = vm.get('record');
